...
location.href+'" and xpath="*"')+"&format=json&callback=?",function(b){c('[data-counter="xi"]').text(a(b.query.results.html.body.div[0].div[0].span.content))})};c(b).on("click",".goodshare ins[data-type]",function(a){a.preventDefault();goodshare.init(this)});c(b).getCount()})})(jQuery,window,document);
